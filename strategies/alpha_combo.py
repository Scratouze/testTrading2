...
class StrategyConfig:
    ...
    adx_len: int = 14
    adx_min: float = 27.0
    be_rr: float = 1.0          # zone BE non-utilisée ici (gardée pour extension)
    slope_lookback: int = 5

    # Sécurité
    min_bars: int = 300

    # Filtres de session / volatilité
    trade_start_hour: int = 13      # 13:00 UTC
    trade_end_hour: int = 22        # 22:00 UTC
    avoid_weekends: bool = True
    atrp_window: int = 200
    atrp_min_quantile: float = 0.35
...
def _adx(df: pd.DataFrame, n: int) -> pd.Series:
    ...
    tr = _atr(df, 1)  # TR simple (non moyenné)
...
class AlphaComboStrategy:
    ...
    def compute_levels(self, sig: Signal, close_px: float, atr: float) -> Dict[str, float]:
        ...
        if sig == "BUY":
            ...
            trail_trigger = close_px + (c.atr_mult_tp * atr) * 0.25  # déclenchement progressif
        else:
            sl = close_px + c.atr_mult_sl * atr
            tp = close_px - c.atr_mult_tp * atr
    ...
    def generate_signals(self, candles: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- Filtre horaire / week-end ---
        hours = df.index.hour
        weekdays = df.index.weekday  # 0=lundi ... 6=dimanche
        in_hours = (hours >= c.trade_start_hour) & (hours <= c.trade_end_hour)
        not_weekend = (weekdays <= 4) if c.avoid_weekends else True

        # --- Filtre de volatilité (ATR au-dessus d'un quantile mobile) ---
        atr_low_thresh = df["atr"].rolling(c.atrp_window, min_periods=c.atrp_window).quantile(c.atrp_min_quantile)
        vol_ok = df["atr"] >= atr_low_thresh.fillna(np.inf)  # bloque tant que pas de seuil

        df["tradable"] = in_hours & not_weekend & vol_ok

        # Signal final (on émet quand même le signal, le backtester vérifiera 'tradable' avant d’entrer)
